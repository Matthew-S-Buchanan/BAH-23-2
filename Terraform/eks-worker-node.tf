...
resource "aws_eks_node_group" "final" {
  cluster_name    = aws_eks_cluster.final.name
  node_group_name = "final"
  node_role_arn   = aws_iam_role.final-node.arn
  ...
  depends_on = [
    aws_iam_role_policy_attachment.final-node-AmazonEKSWorkerNodePolicy,
    aws_iam_role_policy_attachment.final-node-AmazonEKS_CNI_Policy,
    aws_iam_role_policy_attachment.final-node-AmazonEC2ContainerRegistryReadOnly,
  ]
}
